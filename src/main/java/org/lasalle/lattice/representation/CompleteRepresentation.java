...
public class CompleteRepresentation<T,R> implements GraphRepresentation<T,R> {
    @Override
    public Graphviz represent(Lattice<T,R> lattice) {
        return Graphviz.fromString(this.latticeAsDot(ConceptLattice.completeLattice(lattice.getTable())));
    }
}
